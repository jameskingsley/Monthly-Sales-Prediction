...
EXPERIMENT_NAME = "Monthly_Sales_Prediction"
mlflow.set_experiment(EXPERIMENT_NAME)

# ---------------- Load Local Model & Scaler ----------------
MODEL_FILE = "monthly_sales_model.joblib"
...
if os.path.exists(MODEL_FILE) and os.path.exists(SCALER_FILE):
    ...
    st.warning("Model or Scaler not found. Train the model first.")
    st.stop()
...
if st.button("Predict Sales"):
    ...
    # ---------------- Log to MLflow ----------------
    with mlflow.start_run(run_name="Local_Joblib_Prediction"):
        mlflow.log_param("Year", year)
        mlflow.log_param("Month", month)
        mlflow.log_param("lag_1", lag_1)
        mlflow.log_param("lag_2", lag_2)
        mlflow.log_param("lag_3", lag_3)
